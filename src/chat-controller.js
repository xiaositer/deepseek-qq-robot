...
const RUNTIME_RULES = `你正在通过 QQ 聊天。只输出要发送给对方的最终文本，不输出分析过程、规则、工具名或动作说明。普通闲聊优先简短自然；确有必要时才详细说明。用换行表示不同 QQ 消息，最多三条。`;
...
export class ChatController {
  ...
  handle(message) {
    if (!this.#shouldHandle(message)) return Promise.resolve(false);
